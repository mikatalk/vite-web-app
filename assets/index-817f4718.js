...
`;y(document.querySelector("#counter"));"serviceWorker"in navigator&&!/localhost/.test(window.location)&&S();let p;const v=document.querySelector(".add-button");window.addEventListener("beforeinstallprompt",n=>{console.log("INSTALL STARTING!"),n.preventDefault(),p=n,v.style.display="block",v.addEventListener("click",()=>{v.style.display="none",p.prompt(),p.userChoice.then(r=>{r.outcome==="accepted"?console.log("User accepted the A2HS prompt"):console.log("User dismissed the A2HS prompt"),p=null})})});
